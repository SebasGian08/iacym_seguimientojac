				...
				if(empty($_POST['txtNombre']) || empty($_POST['txtDescripcion']) || empty($_POST['listStatus']) )
				{
					...
					if($intIdportada == 0)
					{
						...
						if($_SESSION['permisosMod']['w']){
							$request_portada = $this->model->insertPortada($strNombre, $strDescipcion,$imgPortada,$ruta,$intStatus);
							$option = 1;
						}
					}else{
						//Actualizar
						if($_SESSION['permisosMod']['u']){
							...
							$request_portada = $this->model->updatePortada($intIdportada,$strNombre, $strDescipcion,$imgPortada,$ruta,$intStatus);
							$option = 2;
						}
					}
			...
			if($_SESSION['permisosMod']['r']){
				...
				for ($i=0; $i < count($arrData); $i++) {
					...
					if($_SESSION['permisosMod']['d']){	
						$btnDelete = '<button class="btn btn-danger btn-sm" onClick="fntDelInfo('.$arrData[$i]['idportada'].')" title="Eliminar Portada"><i class="far fa-trash-alt"></i></button>';
					}
